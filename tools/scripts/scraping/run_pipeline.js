...
/*
 ...
 Usage:
   node run_pipeline.js <builderId> <projectId> [--bucket my-bucket] [--env staging|prod] [--public] [--dry-run] [--skip-upload] [--skip-seed]

 Environment Support:
 - Use --env staging or --env prod to automatically select the appropriate storage bucket
 - Looks for environment variables like STORAGE_BUCKET_STAGING, STORAGE_BUCKET_PROD, etc.
 - Can also set SEED_ENV environment variable instead of using --env flag
 ...
 - This script expects the other helper scripts to live at tools/scripts/scraping/generate_manifest.js and tools/scripts/scraping/upload_project_media.js and the seeder at backend/firebasefunctions/scripts/seed_sample_project.ts
*/
...
function usage() {
  console.error('Usage: node run_pipeline.js <builderId> <projectId> [--bucket my-bucket] [--env staging|prod] [--public] [--dry-run] [--skip-upload] [--skip-seed]');
  process.exit(1);
}
...
// If env provided and no explicit bucket, look for STORAGE_BUCKET_<ENV>, GS_BUCKET_<ENV> or GCLOUD_STORAGE_BUCKET_<ENV>
if (!opts.bucket && opts.env) {
  const up = opts.env.toUpperCase().replace(/[^A-Z0-9]/g, '_');
  opts.bucket = process.env[`STORAGE_BUCKET_${up}`] || process.env[`GS_BUCKET_${up}`] || process.env[`GCLOUD_STORAGE_BUCKET_${up}`] || null;
...
(async () => {
  try {
    // 1) generate manifest
    const genScript = path.join(repoRoot, 'tools', 'scripts', 'scraping', 'generate_manifest.js');
    ...
    if (!opts.skipUpload) {
      const uploadScript = path.join(repoRoot, 'tools', 'scripts', 'scraping', 'upload_project_media.js');
