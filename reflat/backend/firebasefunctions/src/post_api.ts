...
// Simple health endpoint to verify secret availability in emulator/production
...
export async function postApiHandler(req: any, res: any): Promise<void> {
  ...
  const isTrustedOrigin = (): boolean => {
    ...
    return origin === "http://localhost:3000" || origin === "https://reflat.web.app";
  };
  ...
  // Body for replace/merge: full object keyed by city -> locality -> { rent: [{ id, name }], resale: [{ id, name }] }
  // POST /api/admin/serviceable_projects/add
  // Body for add: { city, locality, id, name, mode: 'rent'|'resale' }
  if (parts[0] === "admin" && parts[1] === "serviceable_projects") {
    if (!isAdminAuthorized() && !isTrustedOrigin()) { res.status(401).json({ error: "Unauthorized" }); return; }

    const docRef = db.collection("config").doc("serviceable_projects");

    if (parts.length === 2) {
      const merge = String((req as any).query?.merge || "false") === "true";
      const body = (req.body || {}) as any;
      if (typeof body !== "object" || Array.isArray(body)) {
        res.status(400).json({ error: "Body must be an object keyed by city" });
        return;
      }
      if (merge) {
        const snap = await docRef.get();
    ...
    if (parts[2] === "add") {
      ...
      const required = ["city", "locality", "id", "name"] as const;
      for (const k of required) { if (!b?.[k]) { res.status(400).json({ error: `Missing '${k}'` }); return; } }
      const mode = String(b.mode || "rent").toLowerCase();
      ...
      const prj = { id: String(b.id), name: String(b.name) };

      const snap = await docRef.get();
      const doc = snap.exists ? (snap.data() as any) : {};
      doc[city] = doc[city] || {};
      // Backward-compat: if existing value is an array, treat it as 'rent' and wrap
      const existing = doc[city][locality];
      if (Array.isArray(existing)) {
        doc[city][locality] = { rent: existing, resale: [] };
      } else if (!existing) {
        doc[city][locality] = { rent: [], resale: [] };
      }
      const bucket = (doc[city][locality][mode] || []) as Array<{ id: string; name: string }>;
      const idx = bucket.findIndex((p) => p.id === prj.id);
      if (idx >= 0) bucket[idx] = prj; else bucket.push(prj);
      doc[city][locality][mode] = bucket;
