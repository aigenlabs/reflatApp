...
// const FIREBASE_STORAGE_URL = "https://storage.googleapis.com/reflat.firebasestorage.app";
  ...
  //     setError(null);
  //     try {
  //       // 1) get project refs for the locality
  ...
  //       // let locationProjects = result.projects || [];
  ...
  //       // (optional) if current selectedBuilder isn't in this locality, reset it
  //       if (selectedBuilder && !builderOptions.includes(selectedBuilder)) {
  //         setSelectedBuilder("");
  //       }

  //   // 3) now apply the builder filter to decide what to load
  ...
  //   // 4) fetch details for filtered refs
  //   const details = await Promise.all(
  //     refsToLoad.map(p =>
  //       fetch(`${FIREBASE_FUNCTIONS_URL}/project_data/${p.builder_id}/${p.project_id}`)
  //         .then(r => r.json())
  //     )
  //   );

  //       const validProjects = details.filter(Boolean);

  //       // 4) normalize + URLs
  //       const projectsList = validProjects.map((p) => ({
  ...
  //       // 5) derive builder options from the current locality’s results
  //       const uniqBuilders = Array.from(
  //         new Set(projectsList.map((p) => p.builderId))
  //       ).sort();
  //       setBuilderIds(uniqBuilders);
  //     } catch (err) {
  ...
  // disable Builder until there are results for the chosen locality
  // const isBuilderDisabled =
  //   !(selectedCity && selectedLocation && projects.length > 0);
  useEffect(() => {
  async function loadProjects() {
    // Only load when city & location selected
    if (!selectedCity || !selectedLocation) {
  ...
  <div className="topbar__wrap">
    <img
      src="../assets/images/reflat_logo.png"
      alt="ReFlat Logo"
          ...
          {projects.length > 0 ? (
            projects.map((p, idx) => (
              <div className="row row-tight">
                {console.log("XXXX:", projects)}
                {projects.map((p, idx) => (
                  <div className="col-12 col-sm-6 col-lg-4" key={`${p.builderId}-${p.projectId}-${idx}`}>
